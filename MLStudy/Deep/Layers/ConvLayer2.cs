...
{
    public sealed class ConvLayer2 : ILayer, IOptimizable
    {
        ...
        private List<ConvLayer2> mirrorList = new List<ConvLayer2>();
        private int samples;
        ...
        private int filterRowLength;

        public ConvLayer2(int filterCount, int square, int stride, int padding = 0)
            : this(filterCount, square, square, stride, stride, padding, padding)
        { }

        public ConvLayer2(int filterCount, int filterRows, int filterColumns, int rowStride, int columnStride, int rowPadding = 0, int columnPadding = 0)
        {
        ...
        public TensorOld PreparePredict(TensorOld input)
        {
            ...
            inputRows = input.shape[1];
            inputColumns = input.shape[2];
            channels = input.shape[3];
            filterRowLength = FilterColumns * channels;
            ...
            ForwardOutput = new TensorOld(samples, outRows, outColumns, FilterCount);
            if (Filters == null)
                SetFilters(TensorOld.RandGaussian(FilterCount, channels, FilterRows, FilterColumns));
            if (Bias == null)
                SetBias(TensorOld.Zeros(FilterCount));
            PaddingInput = TensorOld.Values(PaddingValue, samples, inputRows + 2 * RowPadding, inputColumns + 2 * ColumnPadding, channels);
            return ForwardOutput;
        }
        ...
        {
            var result = (ConvLayer2)CreateSame();
